...
class Tracker:
    def __init__(
        ...
        self.id = "-".join([str(uuid.uuid4()).split('-')[0], class_ID])
...
class MultiObjectTracker:
    ...
    def step(self, detections: Sequence[Detection], class_list):
        ...
        for det_idx in set(range(len(detections))).difference(assigned_det_idxs):
            tracker = Tracker(box0=detections[det_idx].box,
                              model_spec=self.model_spec, class_ID=class_list[det_idx],
                              **self.tracker_kwargs)
            tracker
