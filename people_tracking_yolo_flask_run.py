...
def draw_text(img, text, above_box, color=(255, 255, 255)):
    ...
    cv2.rectangle(img, (int(above_box[0]), int(above_box[1])), (int(above_box[0])+90, int(above_box[1])-15), (80,80,80), -1)
...
def custom_draw_track(img, track, object_ID, direction_stamp, height, random_color: bool = True, fallback_color=(200, 20, 20)):
    if object_ID == 'person':
        if direction_stamp == 'incoming':
            color = [0, 255, 0]
            th = 3
        elif direction_stamp == 'outgoing':
            color = [0, 0, 255]
            th = 3
        else:
    ...
    if height:
        img = draw_text(img, track[0].split('-')[1] + ' - ' + str(int(round(height))) + 'cm', above_box=track.box)
    else:
        img = draw_text(img, track[0].split('-')[1], above_box=track.box)
...
def save_video(filename, volume, image_shape):
    ...
    out = cv2.VideoWriter(filename + ".mp4", cv2.VideoWriter_fourcc(*'mp4v'), 5, image_shape)
    for frame in volume:
...
def update_status(d, d_bbox, ID, status_ID, height):

    d[ID]['status'] = status_ID
    d[ID]['height'] = height

    # d_bbox.pop(ID, None)
    # del d_bbox[ID]
    return d, d_bbox, height
...
parser.add_argument('-tol', '--tolerance', type=int, default=5)
parser.add_argument('-sc', '--scaling', type=float, default=0.5)
...
def main():
    ...
    elif args.input_video == 'home':
        input_video = 'home-vid.mp4'
        fx, fy = 0.5, 0.5
        x1_loc = 140
        y1_loc = 290
        x2_loc = 200
    ...
    d = {
        'ID': 
            {'object': 'value_1', 'time': 'value_2', 'status': 'value_3', 'height': 'value_4', 'backpack': 'value_5', 'umbrella': 'value_6', 'handbag': 'value_7', 'tie': 'value_8', 'suitcase': 'value_9'}
    }
    ...
    while(True):
        ...
        if args.input_video == 'home':
            # specifically need to crop vertically filmed video
            # too high resolution and too many frames.
            if clear_ctr % 2 == 0:
                continue
        ...
        if len(indices) > 0:
            ...
            for idx, track in enumerate(track_list):
                ...
                if class_ID != 'person':
                    ...
                    custom_draw_track(img, track, 'accessory', 'N', '')
                else:
                    # add to dictionary (changed to list) if it doesn't exist
                    # elif exists(ID, arr_d) is False:
                    x1_update = round(bound_box[0])
                    y1_update = round(bound_box[1])
                    x2_update = round(bound_box[2])
                    y2_update = round(bound_box[3])
                    if ID not in d.keys():
                        d.update({
                        ...
                        d_bbox.update({
                            ID: {'x1': [x1_update], 'y1': [y1_update], 'x2': [x2_update], 'y2': [y2_update]}
                        })

                    # every frame, we append and calculate in or out or if status already determined, we don't need to record
                    if d[ID]['status'] is None:
                        # it's already in the list, we append the position
                        d_bbox[ID]['x1'].append(x1_update)
                        d_bbox[ID]['y1'].append(y1_update)
                        d_bbox[ID]['x2'].append(x2_update)
                        d_bbox[ID]['y2'].append(y2_update)

                    # detect if any are close to the range of doorway:
                    # tuning the horizontal axis (less sensitive than vertical direction)
                    if min([y2_loc,y1_loc]) - tolerance <= y2_update <= max([y2_loc,y1_loc]) + tolerance and min([x1_loc,x2_loc]) - tolerance - 30 <= x2_update <= max([x1_loc,x2_loc]) + tolerance + 30:
                        # if close to doorway, look at previous scans and see average y2 location. If above door, then one way, if below, then other
                        # if only length of one, we can assume they are coming in - otherwise if they are inside, algorithm would've detected.
                        scaled_height = (max([y2_update, y1_update]) - min([y2_update, y1_update])) * scaling
                        if len(d_bbox[ID]['y1']) < 2:
                            direction_stamp = 'incoming'
                        else:
                            if mean(d_bbox[ID]['y1'][1:3] + d_bbox[ID]['y2'][1:3]) < mean([y1_update,y2_update]):
                                # the first element recorded is lower than it is now, meaning that it was higher than it is now, so going down is coming in (y1 is more stable)
                                if in_orientation == 'down':
                                    direction_stamp = 'incoming'
                                else:
                                    direction_stamp = 'outgoing'
                            else:
                                # mean is larger than it is now, so it WAS lower than it is now, so going up is incoming
                                if in_orientation == 'up':
                                    direction_stamp = 'incoming'
                                else:
                                    direction_stamp = 'outgoing'
                        d, d_bbox, height = update_status(d, d_bbox, ID, direction_stamp, scaled_height)
                        custom_draw_track(img, track, 'person', direction_stamp, height)

                    else:
                        custom_draw_track(img, track, 'person', False, d[ID]['height'])
